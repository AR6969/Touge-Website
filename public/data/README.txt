California Touge road collection — September 8–13, 2026
...
Southern California and newer northern traces select connected OSM edges between existing vertices
...
tolerance, retaining shared road endpoints, then coordinates rounded to five decimal places (about 1 metre). The
...
Retrieved via https://overpass-api.de/api/interpreter on September 8–13, 2026.
Build recipes: scripts/build-road-data.py, scripts/build-la-road-data.py,
scripts/build-sierra-road-data.py, or scripts/build-north-road-data.py,
...
Northern additions use data/north-overpass.json and scripts/north-roads.json.
Sierra data uses data/sierra-overpass.json and scripts/build-sierra-road-data.py.
